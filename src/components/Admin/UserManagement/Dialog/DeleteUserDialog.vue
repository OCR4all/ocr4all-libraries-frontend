...
async function deleteUsers(){
  for (const user of data.value.data) {
    await useCustomFetch(`/administration/security/user/remove?login=${user.login}`)
      .json()
      .then((response) => {
...
<template>
  <div class="m-4">
    ...
    <ul class="px-4">
      <li v-for="user in data.data" :key="user.login" class="list-disc font-semibold text-surface-950 dark:text-surface-0">
        {{ user.login }}
      </li>
    </ul>
  </div>
