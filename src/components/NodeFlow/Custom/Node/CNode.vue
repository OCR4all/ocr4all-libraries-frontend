...
import { AbstractNode, GRAPH_NODE_TYPE_PREFIX, IGraphNode } from "baklavajs";
import { useGraph, useViewModel } from "baklavajs";

import ContextMenu from "./CContextMenu.vue";
import VerticalDots from "../icons/VerticalDots.vue";
import NodeInterface from "./CNodeInterface.vue";
...
function getColorByClass(type: string): string {
  switch (type) {
    case "Launcher":
...
<template>
  ...
  >
    ...
    <slot name="title">
      <div class="__title" :class="getColorByClass(node.category)" @pointerdown.self.stop="startDrag" @contextmenu.prevent="openContextMenu">
