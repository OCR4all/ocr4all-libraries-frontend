<script setup lang="ts">
import AppLayout from "@/layout/AppLayout.vue";
</script>

<template>
  <AppLayout>
    <div
      class="flex min-h-screen flex-col items-center justify-center bg-surface-0 dark:bg-surface-900"
    >
      <h1
        class="mb-8 animate-pulse text-5xl font-bold text-surface-900 dark:text-white"
      >
        Coming Soon
      </h1>
      <p class="mb-8 text-lg text-surface-800 dark:text-surface-100">
        Dataset curation and training with
        <span class="font-bold">calamari</span>,
        <span class="font-bold">kraken</span> and
        <span class="font-bold">tesseract</span> is currently being implemented!
      </p>
    </div>
  </AppLayout>
</template>
